...
pif_LongInt/Signed32
	...
	// FIXED_PRECISION and SIGNED_MODE are fixed operation modes that should not be changed. If this
	// library is used correctly (e.g., using only the public methods and not using private members
	// or directly accessing variables) then the fixed flags can't be changed.
			...
			*/
					...
					if(Args.len == 1)
						...
						if(Integer < 0)
							...
							// needing negated back at the end.
	...
	Add(...)
		...
		var
			...
			result_sign // Negative or positive if the result should be negative or positive, and 0
						// if the result could be either. We only check for overflow if result_sign
						// != 0.
		...
		if(overflow_mode)
			// We'll only bother computing result_sign if OVERFLOW_EXCEPTION is enabled.
		...
		if(overflow_mode && (result_sign != 0))
			// If OVERFLOW_MODE is enabled, we need to doublecheck
	...
	Multiply(...)
		...
		*/
		...
		pliADDDATA(buffer, prod_2)
		prod_3 += pliBUFFER(buffer) + pliBUFFER(prod_2)
		pliADDBUFFER(prod_3, prod_4) // Still zero up to this point, and buffer is at most 255,
									 // we don't need to worry about flushing it.
		...
		*/
		...
		if(overflow_flag || !(mode & OVERFLOW_EXCEPTION))
			// If we arent' checking for overflow, then we can directly add the data to prod_4 and
			// flush at the end.
...
Let D(x,y) = (q,r) iff y*q + r = x. Then we have the following where "+" denotes some positive
number and "-" denotes some negative number:
...
That is, if Q(x,y) is the quotient of x and y, and if R(x,y) is the remainder of x and y, and if the
relationship y*Q(x,y) + R(x,y) = x holds for all integers x, y, then we have the following:
...
*/
	...
	Quotient(...)
		...
		if(swap_sign)
			// We converted the source object from negative to positive and it was in NEW_OBJECT
			// mode, so now we have to set it back.
		...
		if(quotient_sign < 0)
			// If quotient_sign is negative, then we have to adjust the sign of Quot correctly.
	...
	Remainder(...)
		var
			...
			// True if the sign of the source object needs to be swapped at the end. This happens if
			// NEW_OBJECT mode is off and the source object was negative at the start.
			swap_sign = 0
		...
		if(Processed[2] & 0x8000)
			// If the largest bit is turned on, then Processed is negative and we need to make it
			// positive.
		...
		// This a highly-abbreviated variation of _AlgorithmD(). It's shorter because we can assume
		// that M = 3, N = 1, and we don't have to worry about finding the remainder (because it's
		// equal to 0).
	...
	Negate()
		// Note that if A = 0x80000000, then A.Negate() == A. This is allowed by the pif_Arithmetic
	...
	Compare(pif_LongInt/Int)
		...
		if(istype(Int))
			...
			if(!(Int.Mode() & SIGNED_MODE))
				...
				// the source data is less than the input data, *but* if we proceed as usually it
				// could on its face appear to be larger due to how two's complement works. E.g., if
				// the input data is equal to 4,294,967,295 and the source data is equal to -1, then
				// without checking the comparison would report htem equal because both have the
				// same representation of 0xFFFFFFFF.
		...
		/*
		 * Comparisons between data of different signs won't work as nicely as when they have the
		 * same sign. Luckily, once we know the sign we can immediately determine which is greater
		 * than and less than.
		 */
		...
		/*
		 * If they do have the same sign, though, we can proceed like we did with the parent method.
		 */
	...
	IsNegative()
		// In two's complement notation, a number is negative if and only if its largest bit is on.
	...
	PrintDecimal()
		...
		if(IsNegative())
			if((block_1 == 0x0000) && (block_2 == 0x8000))
				// This special case has to be taken into account because 0x80000000 is the minimum
				// for a signed double, and is outside the positive range. What happens is that when
				// trying to negate it, you have ~0x80000000 + 1 = 0x7FFFFFFF + 1 = 0x80000000 and
				// thus get nowhere, breaking this method. This is the only value that could cause
				// something like, though, so we only need to take special considerations for it and
				// not any others.
